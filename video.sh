...
while true; do
    ...
    while true; do
        ...
        grep -q "https://login.mos.ru" $resp && echo "Warning: re-login requested">&2 && sleep 10 && break
        m3u=`cat $resp | jq -r '.live | .ios | .url[0]'`
        [ -z "$m3u" ] && echo "Error: couldn't get playlist URL, retrying">&2 && sleep 20 && continue
