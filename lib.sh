...
loginPgu() {
    ...
    curl -s -o /dev/null -c $cjar -b $cjar -A "$ua" -e ';auto' -k -L "$login_url"
    curl -s -o /dev/null -c $cjar -b $cjar -A "$ua" -e ';auto' -k -L --data-urlencode "login=$login" --data-urlencode "password=$password" --data-urlencode "me=on" 'https://login.mos.ru/sps/login/methods/password'
    curl -s -o /dev/null -c $cjar -b $cjar -A "$ua" -e ';auto' -k -L 'https://beta-my.mos.ru/my/'
